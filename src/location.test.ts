...
// 内部 5 producer は geolocation/fetch に依存するため、test 環境(navigator なし)では各々が
// error doc に degrade する。ここでは「2 group(weather/place)へ再編して 1 doc を返す」骨格と
// abort 挙動を検証する(整形ロジックは weather/places 等の個別テストが担う)。
